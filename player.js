...
function handleVideoInput() {
    ...
    if (document.querySelector('.popup-overlay')) {
        showToast('Please handle the current popup first', 'warning');
        return;  // Add this block to prevent multiple popups
    }
}
...
function showLoader() {
    updateProgress(0);  // Already there, but ensures reset
...
function updateProgress(percent) {
    ...
    if (progressBar && progressText) {
        const capped = Math.min(100, Math.max(0, percent));  // Add this to cap between 0-100
...
function showPlaylistPopup(videoId, playlistId) {
    ...
    popup.innerHTML = `
        <div class="popup-content">
            <div class="popup-header">
                <h3><span class="material-symbols-outlined">folder</span> Playlist Detected</h3>
            ...
            <div class="popup-body">
                ...
                <div class="popup-buttons">
                    <button class="btn save-playlist-btn" onclick="saveEntirePlaylist('${playlistId}'); closePopup();">
                            Save Entire Playlist
                    </button>
                    <button class="btn save-video-btn" onclick="saveSingleVideo('${videoId}'); closePopup();">
                            Save Single Video
                    </button>
                </div>
            </div>
    ...
    // Use event delegation – this guarantees the latest function is called
    popup.addEventListener('click', function(e) {
        if (e.target.classList.contains('close-popup') || e.target.parentElement?.classList.contains('close-popup')) {
            closePopup();
            return;
        }

        if (e.target.classList.contains('save-playlist-btn') || e.target.closest('.save-playlist-btn')) {
            e.preventDefault();
            saveEntirePlaylist(playlistId);
        }

        if (e.target.classList.contains('save-video-btn') || e.target.closest('.save-video-btn')) {
            e.preventDefault();
            saveSingleVideo(videoId);
        }
    });
}
function closePopup() {
    document.querySelectorAll('.popup-overlay').forEach(popup => popup.remove());  // Change to querySelectorAll to remove all popups
...
async function saveEntirePlaylist(playlistId) {
    ...
    closePopup();  // Add this line to close the popup after saving
